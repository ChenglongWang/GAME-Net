...
class FlexibleNet(torch.nn.Module):
    def __init__(
        ...
        self.linear_block = torch.nn.ModuleList(
            [Linear(self.dim, self.dim, bias=bias) for _ in range(self.num_linear_layers)]
        )
        self.conv_block = torch.nn.ModuleList(
            [conv(self.dim, self.dim, bias=bias) for _ in range(self.num_conv_layers)]
        )
        if self.adj_conv:
            self.adj_block = torch.nn.ModuleList(
                [Linear(self.dim, self.dim, bias=bias) for _ in range(self.num_conv_layers)]
            )
        self.pool = pool(
            self.dim, k=int(300*pool_ratio), heads=pool_heads,
        )
        self.final_linear = Linear(self.dim, 1)
...
class PreTrainedModel:
    def __init__(self, model_path: str):
        ...
        self.model = torch.load("{}/model.pth".format(self.model_path), map_location=torch.device("cpu"))
        self.model.load_state_dict(torch.load("{}/GNN.pth".format(self.model_path), map_location=torch.device("cpu")))
        self.model.eval()
        self.model.to("cpu")
